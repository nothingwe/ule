...
Pagination.prototype.addInput=function(){
	...
	tiao.innerHTML=`到第 <input type="number" value="${this.default.pageInfo.pagenum}" max="${this.default.pageInfo.totalpage}" min="1">  页 `
	var strong=document.createElement("strong")
	strong.innerHTML=`共${this.default.pageInfo.totalpage}页`
    //创建按钮对象
    var btn=document.createElement('button')
    btn.innerHTML="确定"
    //把当前创建好的两个对象依次追加到大tiao盒子中
	this.ele.appendChild(strong)
    this.ele.appendChild(tiao)
...
Pagination.prototype.jinyong=function(){
    ...
    if(pagenum2==1){
        //禁用首页和上一页
        divs[0].style.color="#ddd"
        divs[1].style.color="#ddd"
    }
    if(pagenum2==totalPage2){
        //禁用下一页和尾页
        divs[3].style.color="#ddd"
        divs[4].style.color="#ddd"
    }
}
//显示文本信息
...
Pagination.prototype.dongcidaci=function(){
    ...
    this.ele.onclick=(e)=>{
        ...
        //点击go按钮
        if(target.innerHTML=="确定" && this.default.pageInfo.pagenum!=target.previousElementSibling.value){
            //修改页码
